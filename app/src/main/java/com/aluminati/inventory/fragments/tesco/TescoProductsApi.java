...
import com.aluminati.inventory.fragments.tesco.objects.TescoProduct;
...
public class TescoProductsApi{

    private String result;
    private CompositeDisposable compositeDisposable;
    private ProductReady productReady;
    private ProductsReady productsReady;
    private final int LIMIT = 1000;


    public TescoProductsApi(String reseult){
        Log.i("TescoProductData", "TescoProductData called");
        this.result = reseult;
        this.compositeDisposable = new CompositeDisposable();
    }

    public TescoProductsApi(){
        this.compositeDisposable = new CompositeDisposable();
    }
    ...
    public void getProducts(String result) {

        if(result.isEmpty()){
            productsReady.getProducts(new ArrayList<>());
        }else {

            GetItemsResultDeserializer getItemsResultDeserializer = new GetItemsResultDeserializer();
            new TescoProductQuery(result)
                    .getSearchResult(LIMIT, getItemsResultDeserializer)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(new SingleObserver<Product>() {
                        @Override
                        ...
                        @Override
                        public void onSuccess(Product product) {
                            productsReady.getProducts(getItemsResultDeserializer.getGetProducts());
                        }
    ...
    public void setProductsReady(ProductsReady productsReady){
...
class GetItemListResultDeserializer implements JsonDeserializer<Product> {
    ...
    public Product deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        ...
        if(itemsJsonArray.size() == 1){
            ...
            final String dep = itemJsonObject.get("superDepartment").getAsString();
            ...
            Log.i("Tesco", name + " " + getDes());
                ...
                tescoProduct = loadProduct(id, img, name, price, description,dep);
            ...
            for (JsonElement itemsJsonElement : itemsJsonArray) {
                ...
                if(weakMatch && alternative == null) {
                    alternative = loadProduct(id, img, name, price, description, dep);
                }
    ...
    private Product loadProduct(String id, String img, String name, String price, String description, String des) {
        Product tescoProduct = new Product();
...
class GetItemsResultDeserializer implements JsonDeserializer<Product> {

    public ArrayList<Product> getProducts;

    public GetItemsResultDeserializer() {
        Log.i("Tesco", "Call from search");
        getProducts = new ArrayList<>();
    }

    public ArrayList<Product> getGetProducts() {
    ...
    public Product deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        ...
        for (JsonElement itemsJsonElement : itemsJsonArray) {
            ...
            if(!cons.contains(name)){
                cons.add(name);
                getProducts.add(new Product(id, img, name, price, description, dep));
            }
...
class TescoProductQuery{
    ...
    private final static String TAG = TescoProductQuery.class.getName();
    ...
    public TescoProductQuery(String des){
        this.des = des;
    }
    ...
    public Single<Product> getSearchResult(int limit, Object typeAdapter){
        return TescoProductQuery
                .getRetrofitInstance(Product.class, typeAdapter)
                .create(TescoProductInfo.class).getProductList(getDes(), Integer.toString(0), Integer.toString(limit));
    }
    ...
    public interface TescoProductInfo {
        ...
        @Headers({"Ocp-Apim-Subscription-Key: cbc1fdf45b5a454cae665a1d34a8a094", "Cache-Control: no-cache"})
        @GET("products")
        Single<Product> getProductList(@Query("query") String query, @Query("offset") String offset, @Query("limit") String limit);
...
class TescoProductData {
    ...
    private final static String TAG = TescoProductData.class.getName();
